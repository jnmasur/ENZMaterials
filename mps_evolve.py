...
def phi_enz(p, nnop, psi, ind, scale):
...
class TEBD:
    ...
    """
    def __init__(self, psi, model, p, phi_func, ind, options):
    ...
    def run(self):
        ...
        if self.ind is not None:
            fps = "-nsites{}-U{}-F{}-a{}-maxdim{}".format(self.p.nsites, self.p.u, self.options["F0"], self.p.a, self.options["trunc_params"]["chi_max"])
    ...
    def update_operators(self, tcurrent):
        ...
        if callable(self.phi_func):
            if self.phi_func.__name__ == "phi_tl":
                ...
                phi = self.phi_func(self.p, self.nnop, self.psi, self.ind, self.scale)
